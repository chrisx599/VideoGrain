...
def register_upblock2d(model):
    ...
    for i, upsample_block in enumerate(model.unet.up_blocks):
        if isinstance_str(upsample_block, "UpBlockPseudo3D"):
    ...
    def up_forward(self):
        def forward(
            ...
            for resnet in self.resnets:
                ...
                res_hidden_states = rearrange(res_hidden_states, "(b f) c h w -> b c f h w", f=video_length)
...
def register_crossattn_upblock2d(model):
    ...
    for i, upsample_block in enumerate(model.unet.up_blocks):
        if isinstance_str(upsample_block, "CrossAttnUpBlockPseudo3D"):
            upsample_block.forward = up_forward(upsample_block)
